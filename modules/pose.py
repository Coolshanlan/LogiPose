    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.circle(img, (int(x_a), int(y_a)), 4, Pose.color, -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 4, Pose.color, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)),
                         (int(x_b), int(y_b)), Pose.color, 3)
...
def get_similarity_score(a, b, threshold=0.4):
    ...
    for kpt_id in range(Pose.num_kpts):
        if a.keypoints[kpt_id, 0] != -1 and b.keypoints[kpt_id, 0] != -1:
            apoint[kpt_id] = np.array(
                [a.keypoints[kpt_id][0]-a.bbox[0], a.keypoints[kpt_id][1]-a.bbox[1]])
            bpoint[kpt_id] = np.array(
                [b.keypoints[kpt_id][0]-b.bbox[0], b.keypoints[kpt_id][1]-b.bbox[1]])
    # abbox=list(a.bbox)
    # bbbox=list(b.bbox)
